...
use think\Model;
use think\Db;
class UserlotterdbModel extends Model
{
    protected $table = 'user_dongbiao';
     ...
     * @param [type] $data [description]
     */
    // public function UserAdddb($data,$lotid)
    // {
    // 	//先添加入库操作
    // 	$dataArr = array(
    // 			'name'=>$data['username'],
    ...
    // 				'lotid' =>    $lotid,
    // 			);
    // 		DB::name("lotuser_dongbiao")->insert($UserDatadb);
    // 		return Db::name("lotuser_dongbiao")->getLastInsID();
    // 	}
    // 	else
    // 	{
    // 		return array("start"=>3006,"msg"=>"用户备份失败");
    // 	}
    // }
    ...
    public function EndUserLotter($userId)
    {
		...
		//查询奖品 概率 数量
    	$LotterData = DB::name("lottery_dongbiao")->select();
    	if($LotterData)
    	{
	...
	{
		$end = Db::name('lottery_dongbiao')->field('num')->where('id',$id)->select();
		if(isset($end[0]['num']))
		{
			return $end[0]['num'];
		}
	...
	{
		return Db::name('lotuser_dongbiao')->insert($data);
	}
	...
	{
		$phone = $this->Ckgetphone($userId);
		$res = Db::name('lotuser_dongbiao')->where('phone',$phone)->find();
		return $res['phone'];
	}
	...
	{
		return Db::name('lottery_dongbiao')->where('id',$id)->setDec('num');
	}
	...
	{
		$end = Db::name('user_dongbiao')->field('phone')->where('user_id',$userid)->select();
		if($end)
		{
			return $end[0]['phone'];
		}
